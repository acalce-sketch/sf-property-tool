...
with col1:
    ...
    map_style = st.radio(
        "Map style", ["Flat (2D)", "3D columns (approx. height)", "3D building footprints (experimental)"],
        index=1, horizontal=True,
        help="Building footprints uses real parcel shapes from a second dataset \u2014 slower and less tested. Switch back to 3D columns if it misbehaves.",
    )
    ...
    if not map_df.empty:
        ...
        if map_style == "3D building footprints (experimental)":
            ...
            if not poly_df.empty:
                ...
                pitch = 45
        elif map_style == "3D columns (approx. height)":
            layer = pdk.Layer(
                "ColumnLayer",
                id="parcels",
                data=map_df,

...
ZONING_DESCRIPTIONS = {
    "RH-1": "Single-family homes only \u2014 one dwelling unit per lot (though SB 9 can allow up to 4 units via a lot split, regardless of this label).",
    "RH-2": "House-scale, up to 2 units per lot.",
    "RH-3": "House-scale, up to 3 units per lot.",
    "RM-1": "Low-density multi-family residential.",
    "RM-2": "Moderate-density multi-family residential.",
    "RM-3": "Medium-high density multi-family residential; taller buildings allowed.",
    "RM-4": "High-density multi-family residential \u2014 the densest purely-residential district.",
    "RC-3": "Combined residential-commercial, medium-high density, ground-floor commercial allowed.",
    "RC-4": "Combined residential-commercial, high density, ground-floor commercial allowed.",
    "NC-1": "Small neighborhood commercial corridor \u2014 local shops with housing above, low height.",
    "NC-2": "Neighborhood commercial corridor \u2014 broader range of shops with housing above.",
    "NC-3": "Moderate-scale neighborhood commercial corridor, taller mixed-use buildings allowed.",
    "NCT": "Neighborhood Commercial Transit district \u2014 higher-density mixed-use near transit, reduced/no parking required.",
    "C-2": "Community business district \u2014 broader commercial uses, moderate height.",
    "C-3": "Downtown commercial core \u2014 high-rise office and residential towers allowed.",
    "M-1": "Light industrial \u2014 limited residential use allowed.",
    "M-2": "Heavy industrial \u2014 residential use generally restricted.",
...
ZONING_COLORS = {
    "RH-1": [86, 156, 214], "RH-2": [86, 156, 214], "RH-3": [86, 156, 214],
    "RM-1": [78, 178, 122], "RM-2": [78, 178, 122], "RM-3": [78, 178, 122], "RM-4": [78, 178, 122],
    "RC-3": [201, 162, 39], "RC-4": [201, 162, 39],
    "NC-1": [201, 100, 59], "NC-2": [201, 100, 59], "NC-3": [201, 100, 59], "NCT": [201, 100, 59],
    "C-2": [180, 70, 160], "C-3": [180, 70, 160],
    "M-1": [120, 120, 120], "M-2": [120, 120, 120], "PDR": [120, 120, 120], "P": [90, 90, 90],
}
ZONING_COLOR_UNKNOWN = [200, 200, 200]
...
with col1:
    color_mode = st.radio("Color map by", ["Land value per sqft", "Zoning district"], horizontal=True)
    if color_mode == "Land value per sqft":
        ...
        st.subheader("Zoning district")
        st.caption("Colored by base zoning category. Use the parcel dropdown below to select one, or check the zoning mix expander for the full breakdown.")
    ...
    if not map_df.empty:
        if color_mode == "Land value per sqft":
            ...
            map_df["color"] = map_df["zoning_code"].apply(
                lambda z: ZONING_COLORS.get(zoning_prefix(z), ZONING_COLOR_UNKNOWN)
            )
            legend_bits = " &nbsp; ".join(
                f"<span style='color:rgb({c[0]},{c[1]},{c[2]})'>\u25cf</span> {grp}"
                for grp, c in [("RH (house-scale)", ZONING_COLORS["RH-1"]), ("RM (multi-family)", ZONING_COLORS["RM-2"]),
                               ("RC (res-commercial)", ZONING_COLORS["RC-3"]), ("NC/NCT (commercial corridor)", ZONING_COLORS["NC-2"]),
                               ("C-2/C-3 (business/downtown)", ZONING_COLORS["C-3"]), ("Industrial/PDR/Public", ZONING_COLORS["M-1"]),
                               ("Unrecognized", ZONING_COLOR_UNKNOWN)]
            )

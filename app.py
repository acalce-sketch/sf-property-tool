...
def rect_footprint(lat, lon, frontage_ft, depth_ft, lot_area_sqft):
    """Builds a simple axis-aligned rectangle sized to the parcel's real dimensions \u2014
    no true orientation or shape detail, just correct scale. Falls back to a square
    footprint from lot_area when frontage/depth aren't available."""
    if frontage_ft and depth_ft and frontage_ft > 0 and depth_ft > 0:
        width, depth = frontage_ft, depth_ft
    elif lot_area_sqft and lot_area_sqft > 0:
        side = math.sqrt(lot_area_sqft)
        width, depth = side, side
    else:
        return None
    ft_per_deg_lat = 364000.0
    ft_per_deg_lon = 364000.0 * math.cos(math.radians(lat))
    hw = (width / 2) / ft_per_deg_lon
    hd = (depth / 2) / ft_per_deg_lat
    return [[lon - hw, lat - hd], [lon + hw, lat - hd], [lon + hw, lat + hd], [lon - hw, lat + hd]]
...
PARCEL_SHAPE_URL = "https://data.sfgov.org/resource/acdm-wktn.json"
POLYGON_PARCEL_CAP = 300  # bounds request size / render load for the experimental real-footprint mode


def parse_multipolygon_outer_ring(wkt):
    """Extracts the outer ring of the first polygon from a MULTIPOLYGON WKT string.
    Simplification: ignores holes and additional polygon parts \u2014 fine for single-building parcels,
    which is the overwhelming majority."""
    if not isinstance(wkt, str) or "MULTIPOLYGON" not in wkt:
        return None
...
def fetch_parcel_shapes(blklots: tuple) -> dict:
    if not blklots:
        return {}, False
    quoted = ",".join(f"'{b}'" for b in blklots)
    params = {
        "$where": f"blklot IN ({quoted})",
        "$select": "blklot, shape, active",
        "$limit": len(blklots) * 3,  # a blklot can have retired duplicates; grab extra and filter client-side
    }
    r = requests.get(PARCEL_SHAPE_URL, params=params, timeout=30)
    r.raise_for_status()
    rows = r.json()
    active_shapes, any_shapes = {}, {}
    for row in rows:
        ring = parse_multipolygon_outer_ring(row.get("shape"))
        if not ring:
            continue
        any_shapes[row["blklot"]] = ring
        if row.get("active") in (True, "true", "True"):
            active_shapes[row["blklot"]] = ring
    if active_shapes:
        return active_shapes, False
    return any_shapes, bool(any_shapes)  # fall back to unfiltered matches if active-status filtering found nothing
...
with col1:
    ...
    map_style = st.radio(
        "Map style", ["Flat (2D)", "3D columns (fixed size)", "3D footprints (scaled to lot size)", "3D real footprints (experimental)"],
        index=2, horizontal=True,
        help="Scaled footprints use each parcel's real frontage/depth for correct size without needing exact shape. Real footprints uses a second, less-tested dataset for the actual recorded shape.",
    )
    pitch = st.slider("Camera tilt (0 = straight down, 80 = near horizon)", 0, 80, 45)
    ...
    if not map_df.empty:
        ...
        map_df["scaled_polygon"] = map_df.apply(
            lambda r: rect_footprint(r["lat"], r["lon"], r.get("lot_frontage"), r.get("lot_depth"), r.get("lot_area")),
            axis=1,
        )
        map_df = map_df[["lat", "lon", "r", "g", "b", "land_per_sqft", "parcel_number", "stories", "elevation", "scaled_polygon"]].reset_index(drop=True)

        if map_style == "3D footprints (scaled to lot size)":
            poly_df = map_df.dropna(subset=["scaled_polygon"]).reset_index(drop=True)
            if poly_df.empty:
                st.info("No lot dimensions available for these parcels \u2014 showing 3D columns instead.")
                layer = pdk.Layer(
                    "ColumnLayer", id="parcels", data=map_df, get_position=["lon", "lat"],
                    get_elevation="elevation", elevation_scale=1, radius=15,
                    get_fill_color=["r", "g", "b", 180], pickable=True, auto_highlight=True, extruded=True,
                )
            else:
                layer = pdk.Layer(
                    "PolygonLayer",
                ...
                map_df = poly_df
        elif map_style == "3D building footprints (experimental)":
            poly_df_raw = map_df.head(POLYGON_PARCEL_CAP).copy()
            poly_df = poly_df_raw.copy()
            fetch_failed = False
            try:
                with st.spinner(f"Fetching real building footprints for up to {POLYGON_PARCEL_CAP} parcels..."):
                    shapes, used_fallback = fetch_parcel_shapes(tuple(poly_df["parcel_number"]))
                if used_fallback:
                    st.caption("Active-status filtering found no matches, so this shows all matched shapes \u2014 a few may reflect retired/historical parcel configurations rather than the current one.")
                poly_df["polygon"] = poly_df["parcel_number"].map(shapes)
                poly_df = poly_df.dropna(subset=["polygon"]).reset_index(drop=True)
                with st.expander(f"Debug: footprint matching ({len(shapes)} of {len(poly_df_raw)} matched)"):
                    sample_ids = list(poly_df_raw["parcel_number"])[:10]
                    st.write("Sample parcel_number values being searched for:", sample_ids)
                    st.write("Sample blklot values actually matched:", list(shapes.keys())[:10])
                    quoted_sample = ",".join(f"'{b}'" for b in sample_ids)
                    debug_r = requests.get(PARCEL_SHAPE_URL, params={
                        "$where": f"blklot IN ({quoted_sample})",
                        "$select": "blklot, shape, active", "$limit": 10,
                    }, timeout=30)
                    st.write(f"Raw API response for those 10 parcel_numbers (status {debug_r.status_code}):", debug_r.json()[:5])
            except Exception as e:
                st.warning(f"Couldn't load real footprints ({e}) \u2014 falling back to 3D columns.")
                fetch_failed = True
            ...
            if not poly_df.empty:
                ...
                map_df = poly_df  # so downstream click-selection matches what's actually rendered
            else:
                if not fetch_failed:
                    st.info("No real footprints matched these parcels \u2014 showing 3D columns instead.")
                layer = pdk.Layer(
                    "ColumnLayer", id="parcels", data=map_df, get_position=["lon", "lat"],
                    get_elevation="elevation", elevation_scale=1, radius=15,
                    get_fill_color=["r", "g", "b", 180], pickable=True, auto_highlight=True, extruded=True,
                )
        elif map_style == "3D columns (fixed size)":
            layer = pdk.Layer(
                "ColumnLayer",
                id="parcels",
                data=map_df,

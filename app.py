...
with col1:
    ...
    if not map_df.empty:
        ...
        map_df = map_df[["lat", "lon", "r", "g", "b", "land_per_sqft", "parcel_number"]].reset_index(drop=True)
        ...
        view_state = pdk.ViewState(
            latitude=map_df["lat"].mean(), longitude=map_df["lon"].mean(), zoom=15, pitch=0
        )
        deck = pdk.Deck(
            layers=[layer],
...
improvement_value = (res_sqft * t["value_per_sqft"]) + (retail_sqft * RETAIL_VALUE_PER_SQFT)
...
st.caption(
    ...
    "(citywide average ~$1,170/sqft), not construction cost \u2014 this matches how Prop 13 actually "
    "assesses for-sale product. These are still planning-level estimates, not a substitute for an "
    "actual appraisal or pro forma."
)
